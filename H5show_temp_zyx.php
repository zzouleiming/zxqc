...
class H5show_temp_zyx extends CI_Controller
{
...
//模板二 行程详情
   public  function show_s($h5_id,$day_order){
   $where.="h5_id=$h5_id AND day_order=$day_order";
...
//     echo "</pre";
        ...
        $this->load->view('h5/temp_late_day',$data);
    }

 //模板二 酒店查看跟多
 public  function hotel($id,$h5_id){
     
     $where.="id=$id AND h5_id=$h5_id";
     $data=$this->User_model->get_select_one($select='*',$where,'v_h5_hotel');
     $data['hotel_images']=  json_decode($data['hotel_image']);
//     echo "<pre>";
//     print_r($data);
//     echo "</pre>";
     $this->load->view('h5/aircrafthotel',$data);
...
//                $data['sta_price']=min($data['sta_price_arr']);
            ...
            //获取模板2 所有酒店信息详情
              $h5_hotel = $this->User_model->get_all('*',array('h5_id'=>$id,'is_show'=>1),'v_h5_hotel');
              $data['h5_id']=$id;
              foreach ($h5_hotel as $k1=>$v1){
                  $h5_hotel[$k1]['hotel_images']= json_decode($v1['hotel_image']);
                  
              }
              $data['hotel_list']=$h5_hotel;
        //echo'<pre>';print_r($data['sign_to_know']);exit();
//           echo "<pre>";
//        print_r($data); exit;
        $this->load->view('h5/temp_index',$data);
        $this->show_count();
    }
...
//  $day_info = $this->User_model->get_select_all('*',$where,'day_order','ASC','v_h5_day');
        ...
        $data['car_info']= $this->User_model->get_select_all('*',$where,'Id','ASC','v_h5_car');
        $data['hotel']=$this->User_model->get_select_all('*',$where,'id','ASC','v_h5_hotel');
        $data['package']=$this->User_model->get_select_all('*',$where,'id','ASC','v_h5_package');
         $data['fly']=$this->User_model->get_select_all('*',$where,'fly_id','ASC','v_h5_fly');
     ...
     //echo "</pre>";
          ...
          $this->load->view('zyx/index',$data);  
     }
    //自由行2 机酒套餐详情
    public  function jiudian($id,$h5_id){
            $where='Id='.$id;
            $where1="h5_id=$h5_id  AND type_bang=1 AND type>0";
           $data['hangban']=$this->User_model->get_select_all('*',$where1,'fly_id','ASC','v_h5_fly');
          $data['package']=$this->User_model->get_select_all('*',$where,'id','ASC','v_h5_package');  
          foreach($data['package'] as $k=>$v ){
              $data['package'][$k]['img']=  json_decode($v['hotel_img']);
              
          }
          foreach($data['hangban'] as $k1=>$v1){
              $data['hangban'][$k1]['ks_time']=  date('H:i',$v1['fly_start_time']);
              $data['hangban'][$k1]['dd_time']=  date('H:i',$v1['fly_end_time']);
             
              
          }
      
        $this->load->view('zyx/package',$data);
    }
    //自由行2 酒店详情
    public function zyx_hotel_xq($id){
...
//      print_r($data);
//       echo "</pre>";
        $this->load->view('zyx/hotel_xq',$data);
    }
