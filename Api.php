...
class Api extends My_Controller {
	...
	public function index()
	{
		...
		$data['url'][$i]['src']  = '/index.php/Api/register1?type=weixin&openid=1222&headimgurl=http://wx.qlogo.cn/mmopen/LIUI5tJGiauB3z4rp4zYXCssMJGoP0SEhcgolZYqmVU4Mn2r6J1zbtCRJSxm8hhWiatt1e50jmvbS8P4g9iaxzMuA/0&nickname=cef&sex=1&sys=1&device_id=wfawfawfawfawfawefawfawefa';
		$i++;
		$data['url'][$i]['info'] = '短信验证码获取';
		$data['url'][$i]['src']  = '/index.php/Api/message?mobile=13556121245';
		$i++;

		$data['url'][$i]['info'] = '直播开始';
		$data['url'][$i]['src']  = '/index.php/Api/show_start?user_id=166&title=jaijfa&video_name=166_1460069722&adv_id=111';
		$i++;
		$data['url'][$i]['info'] = '直播结束  用户id     视屏id';
		$data['url'][$i]['src']  = '/index.php/Api/show_stop?user_id=4&video_id=8&praise=12&count=2&creates=22';
		$i++;
		/*$data['url'][$i]['info'] = '点赞';
		$data['url'][$i]['src']  = '/index.php/Api/praise?praise_id=1';
		$i++;*/
		...
		$data['url'][$i]['src']  = '/index.php/api/advance_del?adv_id=33';
		$i++;
		$data['url'][$i]['info'] = '个人信息';
		$data['url'][$i]['src']  = '/index.php/Api/user_info?user_id=2';
		$i++;
		$data['url'][$i]['info'] = '上传头像  (图片要用file传  参数名为files)';
		$data['url'][$i]['src']  = '/index.php/api/avatar_upload?user_id=1&files=dizhi';
		$i++;
		$data['url'][$i]['info'] = '修改昵称';
		$data['url'][$i]['src']  = '/index.php/api/update_user?user_id=1&user_name=大脑袋兔兔';
		$i++;
		$data['url'][$i]['info'] = '修改性别';
		$data['url'][$i]['src']  = '/index.php/api/update_user?user_id=1&sex=男';
		$i++;
		$data['url'][$i]['info'] = '修改个性签名';
		$data['url'][$i]['src']  = '/index.php/api/update_user?user_id=1&pre_sign=大脑袋兔兔 个性签名';
		$i++;
		$data['url'][$i]['info'] = '举报';
		$data['url'][$i]['src']  = '/index.php/api/report?user_id=1';
		$i++;
		$data['url'][$i]['info'] = '拉黑';
		$data['url'][$i]['src']  = '/index.php/api/defriend_add?user_id=1&defriend=2';
		$i++;
		$data['url'][$i]['info'] = '拉黑列表';
		$data['url'][$i]['src']  = '/index.php/api/defriend_list?user_id=1&page=1';
		$i++;
		$data['url'][$i]['info'] = '取消拉黑';
		$data['url'][$i]['src']  = '/index.php/api/defriend_del?user_id=1&defriend=2';
		$i++;
		$data['url'][$i]['info'] = '发送私信';
		$data['url'][$i]['src']  = '/index.php/Api/letter_add?to=1&from=2&content=%E6%B5%8B%E8%AF%95';
		$i++;
		$data['url'][$i]['info'] = '私信列表';
		$data['url'][$i]['src']  = '/index.php/Api/letter_list?user_id=1&page=1';
		$i++;
		$data['url'][$i]['info'] = '私信详细';
		$data['url'][$i]['src']  = '/index.php/Api/letter_info?to=1&from=2';
		$i++;
		$data['url'][$i]['info'] = '删除私信';
		$data['url'][$i]['src']  = '/index.php/Api/letter_del?to=1&from=2';
		$i++;
		$data['url'][$i]['info'] = '禁言';
		$data['url'][$i]['src'] = '/index.php/Api/gag_add?video_id=5&gag_id=2';
		$i++;
		$data['url'][$i]['info'] = '取消禁言';
		$data['url'][$i]['src']  = '/index.php/Api/gag_del?video_id=5&gag_id=2';
		$i++;
		$data['url'][$i]['info'] = '是否禁言';
		$data['url'][$i]['src']  = '/index.php/Api/is_gag?video_id=5&gag_id=2';
		$i++;
		$data['url'][$i]['info'] = '加关注';
		$data['url'][$i]['src']  = '/index.php/Api/follow_add?fans_id=5&user_id=2';
		$i++;
		$data['url'][$i]['info'] = '关注列表  0 接收推送   1  不接受';
		$data['url'][$i]['src']  = '/index.php/Api/follow_list?user_id=2&page=1';
		$i++;
		$data['url'][$i]['info'] = '取消关注列表 ';
		$data['url'][$i]['src']  = '/index.php/Api/follow_del?fans_id=5&user_id=2';
		$i++;
		$data['url'][$i]['info'] = '判断用户是否关注主播 fans_id  是用户id    user_id 主播id   video_id 视屏id';
		$data['url'][$i]['src']  = '/index.php/Api/is_follow?fans_id=5&user_id=2&video_id=5';
		$i++;
		$data['url'][$i]['info'] = '加粉';
		$data['url'][$i]['src']  = '/index.php/Api/fans_add?fans_id=5&user_id=2';
		$i++;
		$data['url'][$i]['info'] = '粉丝列表  ';
		$data['url'][$i]['src']  = '/index.php/Api/fans_list?user_id=2&page=1';
		$i++;
		$data['url'][$i]['info'] = '取消关注粉 ';
		$data['url'][$i]['src']  = '/index.php/Api/fans_del?fans_id=5&user_id=2';
		$i++;
		$data['url'][$i]['info'] = '判断用户是否粉主播 fans_id  是用户id    user_id 主播id ';
		$data['url'][$i]['src']  = '/index.php/Api/is_fans?fans_id=5&user_id=2';
		$i++;
		$data['url'][$i]['info'] = '视屏列表  (type new  最新     hot 热门)';
		$data['url'][$i]['src']  = '/index.php/api/video_list?type=hot';
		$i++;
		$data['url'][$i]['info'] = '关注视屏列表';
		$data['url'][$i]['src']  = '/index.php/api/fans_video?user_id=2';
		$i++;
		$data['url'][$i]['info'] = '观看视屏';
		$data['url'][$i]['src']  = '/index.php/api/watch_start?video_id=2&user_id=2';
		$i++;
		$data['url'][$i]['info'] = '停止观看视屏';
		$data['url'][$i]['src']  = '/index.php/api/watch_stop?video_id=2&user_id=2';
		$i++;
		$data['url'][$i]['info'] = '正在观看人数和观看过的人数(10秒钟请求一次)';
		$data['url'][$i]['src']  = '/index.php/api/watch_num?video_id=2';
		$i++;
		$data['url'][$i]['info'] = '排行标题';
		$data['url'][$i]['src']  = '/index.php/api/rank_info';
		$i++;
		$data['url'][$i]['info'] = '排行榜 (1 时 2 日 3 周 4 月  5  年)';
		$data['url'][$i]['src']  = '/index.php/api/rank_list?type=5&user_id=2&page=1';
		$i++;
		$data['url'][$i]['info'] = '推送测试';
		$data['url'][$i]['src']  = '/index.php/api/pushinfo';
		$i++;
		$data['url'][$i]['info'] = '推送列表';
		$data['url'][$i]['src']  = '/index.php/api/pushlist';
		$i++;
		$data['url'][$i]['info'] = '关闭所有关注者推送';
		$data['url'][$i]['src']  = '/index.php/api/close_push?user_id=2&type=all';
		$i++;
		$data['url'][$i]['info'] = '关闭单个关注者推送';
		$data['url'][$i]['src']  = '/index.php/api/close_push?user_id=2&close_id=3';
		$i++;
		$data['url'][$i]['info'] = '开启所有关注者推送';
		$data['url'][$i]['src']  = '/index.php/api/open_push?user_id=2&type=all';
		$i++;
		$data['url'][$i]['info'] = '开启单个关注者推送';
		$data['url'][$i]['src']  = '/index.php/api/open_push?user_id=2&close_id=3';
		$i++;
		...
		$data['url'][$i]['src']  = '/index.php/api/test_sql?t=v_rank_day';*/
		$i++;
		$data['url'][$i]['info'] = '搜索';
		$data['url'][$i]['src']  = '/index.php/api/search?keyword=美女';
		$i++;
		...
		$data['url'][$i]['src']  = '/index.php/api/version';
		$i++;
		$data['url'][$i]['info'] = '用户反馈 user_id 当前用户id  content 内容' ;
		$data['url'][$i]['src']  = '/index.php/api/response?user_id=1&content=意见建议';
		$i++;
		$data['url'][$i]['info'] = '公共参数';
		$data['url'][$i]['src']  = '/index.php/api/com_public';
		$i++;
		$data['url'][$i]['info'] = '查询排行榜位置  type (1 时 2 日 3 周 4 月  5  年)';
		$data['url'][$i]['src']  = '/index.php/api/rank_user?user_id=2&type=1';
		$i++;
		$data['url'][$i]['info'] = '返回引导页第三方信息接口';
		$data['url'][$i]['src']  = '/index.php/api/thrid_party';
		$i++;
		$data['url'][$i]['info'] = '判断流是否存在 0  不存在  1  存在 ';
		$data['url'][$i]['src']  = '/index.php/api/rtmp_status?video_id=1';
		$i++;
		$data['url'][$i]['info'] = '签名 appid 现在默认为appkey    sign 客户端自动生成值   timestamp 时间戳';
		$data['url'][$i]['src']  = '/index.php/api/appid=appkey&sign=123456&timestamp=1458306639';
...
//		}
		...
		{   
			...
			$data['sign'] = $app_pay->getSign($app_pay->parameters);
			//$this->data_back($data, '0x000');  //返回数据
			$this->User_model->update_one(array('order_id' => $order_id), $data, 'v_order_info');
		//$this->User_model->user_insert('wx_order_info',$data);
			// echo "<pre>";
			// print_r($data);
			 //echo "</pre>";
			 $data['order_id']=$order_id;
			 $data['url']=base_url('Goodsforcar/wx_order_in_xj');
        $this->load->view('zxqcny1/cs_wx',$data);
