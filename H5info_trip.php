...
class H5info_trip extends CI_Controller
{
    ...
    public function index($h5_id=0,$day_order=0)
    {
        ...
        $data['sub_url'] = base_url('h5info_trip/upload');
        $this->load->view('h5/trip_index',$data);
    }
    ...
    public function trip_list($h5_id)
    {
        if($h5_id)
        {
            ...
            if($data['info'])
            {
                ...
                {
                    $data['info'][$k]['edit_url']=base_url("h5info_trip/trip_edit/$v[h5_id]/$v[day_id]");
                    $data['info'][$k]['detail_url']=base_url("h5show/show/$v[day_id]");
                    $data['info'][$k]['del_url']=base_url("h5info_trip/del_day/$v[h5_id]/$v[day_id]");
                    $day_order = $v['day_order'];
                }
            }
            $day_order = intval($day_order)+1;
            $data['add_url']=base_url("h5info_trip/index/$h5_id/$day_order");
            $this->load->view('h5/trip_list',$data);
        }
        else
        {
            redirect(base_url("h5info/h5list"));
    ...
    public function trip_edit($h5_id,$day_id)
    {
        ...
        {
            redirect(base_url("h5info_trip/trip_list/$h5_id"));
        }
        $data['h5_id'] = $h5_id;
        $data['day_id'] = $day_id;
        $data['sub_url'] = base_url('h5info_trip/update');
      //  echo '<pre>';
     //  print_r($data);
        $this->load->view('h5/trip_edit',$data);
    }
    ...
    public function upload()
    {
       ...
       // die();
        ...
        redirect(base_url("h5info_trip/trip_list/$trip_data[h5_id]"));
    }
