...
class H5infot_temp extends CI_Controller
{
...
// 'time'=>strtotime('+1 month -1 day +23 hour', mktime(0,0,0,date('n',$time),1,date('Y',$time))),
        ...
        $data['sub_url']=base_url('h5infot_yl/to_insert');

        $this->load->view('h5/index',$data);
    }
//游轮页面增加
    public function youlun_add()
    {
...
// 'time'=>strtotime('+1 month -1 day +23 hour', mktime(0,0,0,date('n',$time),1,date('Y',$time))),
        ...
        $data['sub_url']=base_url('h5infot_yl/to_insert');
        $this->load->view('h5/youlun',$data);
    }

//模板二
        public function temp_late()
    {
...
// 'time'=>strtotime('+1 month -1 day +23 hour', mktime(0,0,0,date('n',$time),1,date('Y',$time))),
        ...
        $data['sub_url']=base_url('h5infot_temp/to_insert');
        $this->load->view('h5/template',$data);
    }

  //测试新疆页面
      public function temp_lat_xj()
    {
...
// 'time'=>strtotime('+1 month -1 day +23 hour', mktime(0,0,0,date('n',$time),1,date('Y',$time))),
        ...
        $data['sub_url']=base_url('h5infot_temp/to_insert');
        $this->load->view('h5/template2_xj',$data);
    } 
  //二销页面增加
      public function ex_info()
    {
...
// 'time'=>strtotime('+1 month -1 day +23 hour', mktime(0,0,0,date('n',$time),1,date('Y',$time))),
        ...
        $data['sub_url']=base_url('h5infot_temp/to_insert');
        $this->load->view('h5/ex_info',$data);
    } 
...
// 'time'=>strtotime('+1 month -1 day +23 hour', mktime(0,0,0,date('n',$time),1,date('Y',$time))),
        ...
        $data['sub_url']=base_url('h5infot_yl/to_sub');
    ...
    public function h5list()
    {
        ...
        foreach($data['info'] as $k=>$v)
        {
            $data['info'][$k]['edit_url']=base_url("h5info_yl/edit_index/$v[h5_id]");
              if($data['info'][$k]['youlun_id']!=0){
            $data['info'][$k]['edit_url']=base_url("h5info_yl/youlun_inset/$v[h5_id]");
            }else{
             $data['info'][$k]['edit_url']=base_url("h5info_yl/edit_index/$v[h5_id]");
            ...
            if($data['info'][$k]['youlun_id']!=0){
            $data['info'][$k]['youlun_url']=base_url("h5show_yl/youlun_index/$v[h5_id]");
            }
    ...
    //   exit;
        ...
        public function h5list_youlun()
    {
        $where="is_show=1 AND youlun_id=1";
        $data['info']=$this->User_model->get_select_all($select='*',$where,'h5_id','ASC','v_h5_info');
        // echo '<pre>';
        //  print_r(json_encode($data));exit();
        foreach($data['info'] as $k=>$v)
     ...
     // echo "</pre>";
    //   exit;
        $this->load->view('h5/h5list_youlun',$data);
    }
       ...
       // $data['hotel_info']=json_encode($hotel_info);
        ...
        if(count($hotel_easy_title)>=1)
        {
            for($i=0;$i<count($hotel_easy_title);$i++)
            {
             ...
             //   $hotel_img = $this->merge_image($trip_info['day_view'][$i]->view_image,$del_image,$new_url);
                //var_dump($view_image);
                $view_info[] = array(
...
//首页特色展示图
//        $youlun_info = array();
//        $hotel_file_name='hotel_imges1';
//        $hotel_imgs_url=$this->more_pic_upload($hotel_file_name);
//        print_r($hotel_imgs_url); 
//        $youlun_info = array(
//                        'hotel_name' => $this->input->post('hotel_name',TRUE),
//                        'hotel_img'  => $hotel_imgs_url
//            );
//  
        $names=array();
        $cabin=array();
...
////        $cabin['taocan_name'] =$this->input->post('taocan_name',TRUE);  
////        $cabin['taocan_sum'] =$this->input->post('taocan_sum',TRUE);  
////        $cabin['taocan_gg'] =$this->input->post('taocan_gg',TRUE);  
////        $cabin['taocan_money'] =$this->input->post('taocan_money',TRUE); 
////        $cabin['img'] =$view_info;  
...
//           $this->User_model->user_insert('v_h5_youlun',array('h5_id'=>$h5_id,'Cabin_info'=>$cabin_xinxi,'home_details'=>$home_xinxi));
...
//        $hotel_tips=$this->input->post('hotel_tips',TRUE); 
        ...
        foreach($hotel_easy_title as $k=>$v)
        {
            ...
            $hotel_info=array('hotel_easy_title'=>$v,
                ...
                'h5_id'=>$h5_id,
                'hotel_image'=>json_encode($vc=array(
                  'taocan_name'=>$taocan_name[$k],
                    'taocan_sum'=>$taocan_sum[$k],
                    'taocan_gg'=>$taocan_gg[$k],
                    'taocan_money'=>$taocan_money[$k],
                    'img'=>$img[$k],
...
//              echo "</pre>";die;
        ...
        redirect(base_url("h5info_temp/h5list"));
    ...
    public function to_sub()
    {
      ...
      //  $this->User_model->update_one(array('h5_id'=>$h5_id),array('is_show'=>'2'),'v_h5_hotel');
        ...
        redirect(base_url("h5info_yl/edit_index/$h5_id"));
...
//不确定图片上传
    ...
    public function fly_list()
    {
      ...
      //  print_r(json_encode($data));exit();
        foreach($data['info'] as $k=>$v)
        {
            $data['info'][$k]['url']=base_url("h5info_yl/fly_temp?fly_id=$v[fly_id]");
            $data['info'][$k]['del_url']=base_url("h5info_yl/fly_del?fly_id=$v[fly_id]");
        }
    ...
    public function fly_del()
    {
        ...
        redirect(base_url('H5info_yl/fly_list'));
    }
    ...
    public function fly_temp()
    {
       $data['fly_id']=$this->input->get('fly_id',TRUE);
        if($data['fly_id'])
        {
            ...
            $data['sub']=base_url('H5info_yl/fly_temp_sub');
            $this->load->view('h5/fly_temp_edit',$data);
        }
        else
        {
            $data['fly_id']=0;
            $data['sub']=base_url('H5info_yl/fly_temp_insert');
    ...
    public function fly_temp_sub()
    {
        ...
        return redirect(base_url('H5info_yl/fly_list'));
