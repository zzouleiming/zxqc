...
class Api extends My_Controller {
	...
	*/
	public function get_publish()
	{
		...
		//cdn测试
		if($user_id=='1085' || $user_id=='1744' || $user_id=='4011' || $user_id=='1743')
		{
			$data['publish_url'] = 'rtmp://push.etjourney.com/live/'.$video_name;
			//$data['publish_url'] = 'rtmp://121.40.40.61/hls/'.$video_name;
		}
	...
	*/
	...
	{	
        ...
        $userdevice=trim($this->input->post_get('userdevice',TRUE));
        if(!$userdevice)
        {
            $userdevice=0;
        }
		...
		if($type == 'weixin' || $type == 'qq' || $type == 'twitter' || $type == 'facebook' || $type == 'weibo')
		{
			...
			switch ($type) {
				...
				case 'qq':
					$regist_type = '3';
                    $num=strrpos($headimgurl,'/');
                    $headimgurl=substr($headimgurl,0,$num).'/0';
					break;
				case 'facebook':
					$regist_type = '4';
					break;
			...
			{
                $rs=$this->User_model->get_select_one('userdevice',array('openid'=>$openid,'openid!='=>'oj1b8sjQtdu5aYMejjlqadJfxfeg'),'v_users');
                if($rs!=0)
                {
                    if($rs['userdevice']!=0)
                    {
                        if($userdevice!=$rs['userdevice'])
                        {
                            $this->data_back('非绑定设备','0X001','fail');exit();
                        }
                    }else{
                        $this->User_model-> update_one(array('openid'=>$openid),array('userdevice'=>$userdevice),$table='v_users');
                    }
                }
	...
	*/
	public function common_register()
	{
		...
		$tel = isset($_REQUEST['tel']) ? htmlspecialchars(trim($_REQUEST['tel'])) : '';
		...
		if($count)
		{
			$this->data_back('该手机号码已注册','0X011','fail');
		}
	...
	*/
	public function common_login()
	{
		...
		$userinfo = $this->User_Api_model->comment_select('user_id,password,openid,userdevice'," account='$user_name' AND regist_type='6' ",'','',0,1,'v_users');
       // echo '<pre>';print_r($userinfo);
		...
		else
		{
            ...
            if($userdevice && $userinfo[0]['user_id']!='1077' && $userinfo[0]['user_id']!='2025' && $userinfo[0]['user_id']!='4011')
            {
                if($userinfo[0]['userdevice']!='0')
                {
                    if($userdevice!=$userinfo[0]['userdevice'])
                    {
                        $this->data_back('非绑定设备登录','0X011','fail');
                        exit();
                    }
                }
            }
        ...
        {
            if($userinfo[0]['userdevice']=='0'){
                $this->User_model->update_one(array('user_id'=>$data['user_id']),array('userdevice'=>$userdevice),$table='v_users');
            }
        }
	...
	*/
	public function get_tel_verification()
	{
		...
		if(!preg_match("/^13[0-9]{1}[0-9]{8}$|15[0-9]{1}[0-9]{8}$|17[0-9]{1}[0-9]{8}$|18[0-9]{1}[0-9]{8}$/",$tel))
		{   
		...
		$last_time = $time - 60;
        //查询验证码是否已存在
        $verif_info = $this->User_Api_model->comment_select('id,user_id,tel,verification,create_time'," user_id='$user_id' AND tel='$tel' AND verif_type='$verif_type' AND is_checked=0 AND create_time>'$last_time' ",'',' id DESC ',0,1,'v_tel_verification');
	...
	*/
	...
	{  
		...
		if($video_id)
		{
			...
			if($user_id=='1085' || $user_id=='1744' || $user_id=='4011' || $user_id=='1743')
			{
				$rtmp = 'rtmp://pull-rtmp.etjourney.com/live/'.$video_name;
			}
	...
	*/
	public function user_info()
	{
		...
		else
		{
			...
			if($data['is_guide'] == '1')
			{
				$res = $this->User_Api_model->comment_select(' id_range '," user_id=$user_id  ",'','',0,1,'v_auth_views');
				if($res)
				{
			...
			{
				$res = $this->User_Api_model->comment_select(' id_range '," user_id=$user_id  ",'','',0,1,'v_auth_locals');
				if($res)
				{
			...
			if($data['is_driver'] == '1')
			{
				$res = $this->User_Api_model->comment_select(' id_range '," user_id=$user_id ",'','',0,1,'v_auth_drivers');
				if($res)
				{
			...
			{
				$res = $this->User_Api_model->comment_select(' id_range '," user_id=$user_id  ",'','',0,1,'v_auth_business');
				if($res)
				{
	...
	*/
	public function avatar_upload()
	{
		//vars($_FILES['files']);
		$user_id = isset($_REQUEST['user_id']) ? intval($_REQUEST['user_id']) : 0;
		if(!isset($_FILES['files']) OR $_FILES['files']['error']!='0')
		{
			$this->data_back("图片为空", '0x016','fail');
		}
		$name1 = $user_id.'_avatar_big';
		$image_ori = $this->uploadimg->upload_image($_FILES['files'],'user',$name1);
		$name2 = $user_id.time().'_avatar_small';
		$image = $this->thumb($image_ori,$name2);
        $image_pc=$this->imagecropper($image_ori,$user_id);
    ...
    //图片剪切
    public function imagecropper($source_path='./public/images/1265/id_driver.jpg',$user_id,$key2='time',$target_width='220', $target_height='220')
    {
        ...
        if (!file_exists('./public/images/user_head/' . $user_id)) {
            if (!mkdir('./public/images/user_head/' . $user_id, 0777)) {
                return FALSE;
            }
        }
        // 源图过高
        ...
        if ($source_mime == 'image/jpeg') {
            ...
            $new_image = './public/images/user_head/' . $user_id . '/' . $key2 . '.' . $type;
	...
	*/
	...
	public function report()
	{
		...
		if(empty($user_id) OR $report_user_id OR $video_id)
		{
			$this->data_back('参数为空','0X011','fail');
		}
	...
	*/
	public function letter_list()
	{
        ...
        $data=$this->Letter_model->get_letter_list($user_id,$this->config->item('page_num'),$start);
...
//			$this->config->item('catch'),$user_id.$start. 'letter_list',$this->config->item('catch_time'));
	...
	*/
	...
	{	
		...
		if($video_info)
		{
			...
			{
				$image = '/opt/nginx/html/zxqc/api'.ltrim($video_info['image'],'.');
	...
	**/
	...
	{
		$data = $this->User_Api_model->comment_select('title,image_url,link_url'," status=0 AND type='0' AND range_id='0' ",'','displayorder',0,10,'v_banner');
		if($data)
		{
			foreach($data as $key => $value)
			{
	...
	public function my_auth()
	{
		...
		$data = array();
		//认证导游信息
		$res = $this->User_Api_model->comment_select(" id_range,id_view_status "," user_id=$user_id AND id_view_status>'0' ","","id_auth_time DESC",0,1,'v_auth_views');
		if($res)
		{
			if($to != 'zh')
			{
		...
		//认证地陪信息
		$res = $this->User_Api_model->comment_select(" id_range,id_local_status "," user_id=$user_id AND id_local_status>'0' ","","id_auth_time DESC",0,1,'v_auth_locals');
		if($res)
		{
			if($to != 'zh')
			{
		...
		//认证司机信息
		$res = $this->User_Api_model->comment_select(" id_range,id_driver_status "," user_id=$user_id AND id_driver_status>'0' ","","id_auth_time DESC",0,1,'v_auth_drivers');
		if($res)
		{
			if($to != 'zh')
			{
		...
		//认证商户信息
		$res = $this->User_Api_model->comment_select(" id_range,id_business_status "," user_id=$user_id AND id_business_status>'0' ","","id_auth_time DESC",0,1,'v_auth_business');
		if($res)
		{
			if($to != 'zh')
			{
	...
	public function get_position()
	{
		...
		$time = time();
		$date = date("Y-m-d");
		file_put_contents('/opt/nginx/html/zxqc/api/logfile/getposition'.$date.'.log','[time='.$time.'][user_id='.$user_id.'][dimension='.$dimension.'][longitude='.$longitude.']'.PHP_EOL,FILE_APPEND);
		$data['country'] = '';
		$data['city'] = '';
		$data['show_status'] = '1';
		$data['msg_info'] = '';
		...
		file_put_contents('/opt/nginx/html/zxqc/api/logfile/getposition'.$date.'.log','[country='.$data['country'].'][city='.$data['city'].']'.PHP_EOL,FILE_APPEND);

        if($user_id==1742)
        {
           // $json=json_encode($data);
            //$this->User_model->user_insert('v_ts_istype',array('test'=>$json));
        }
        $this->data_back($data, '0x000');  //返回数据
	}
	...
	**/
	public function get_wxpay()
	{
		...
		{
			//新商户（柯邑）
			require_once("./application/third_party/wxpay/WxPayAppNew.php");
		}
		else
		{
			//老商户（迈邑）
...
//0 不显示 1显示
	...
	public function get_location_con_and_city()
	{
		...
		else
		{
			...
			$newarr[0][0]['list']=$this->User_model->get_select_all($select='name,name_en,name_pinyin,lat,lng,lat AS latitude,lng AS longitude,image,id AS range_id ',$where2,$order_title='name_pinyin',$order='ASC',$table='v_location');
            ...
            foreach($newarr[0][0]['list'] as $k=>$v)
            {
               // $is_rs=$this->User_model->get_select_one('act_id',"is_show='1' AND special >'2' AND range_name LIKE '%$v[name]%'",'v_activity_children');
                ...
                $newarr[0][0]['list'][$k]['name_base']=base64_encode($v['name_en']);
            }


		}
	//	$arr=$this->User_model->get_select_all($select='name,level,pid,image,name_en,name_pinyin,lat,lng,image,id AS range_id ',$where1,$order_title='name_pinyin',$order='ASC',$table='v_location');
            ...
            foreach($arr as $k=>$v)
            {
                ...
                //if($is_rs!=0 OR count($is_rs)>1){
                    ...
                    $newarr[1][$str]['list'][]=array(
                        ...
                        'latitude'=>$v['lat'],
                        'longitude'=>$v['lng'],
                        'name_base'=>base64_encode($v['name_en']),
                    );
                //}
...
//获取收藏的产品和商铺和旅拍
	...
	public function get_favorite()
	{
		...
		if($type==1)
		{
            $where="v_favorite.user_id=$user_id AND v_favorite.type ='1'  AND v_goods.is_show='1'";
          //  $where.="  AND v_goods.is_show='1'";
			$select="v_favorite.user_id,v_favorite.act_id,v_activity_children.banner_product as banner_image,v_activity_children.title,v_activity_children.tag,v_activity_children.order_sell AS goods_buy,
			v_goods.shop_price,v_goods.goods_id,v_goods.ori_price";
			...
			foreach($data['list'] as $k=>$v)
			{
				...
				$data['list'][$k]['product_url']=base_url("myshop/products_detail_new?act_id=$v[act_id]&menu=1");
			}


            $where="v_favorite.user_id=$user_id AND v_favorite.type ='3' AND v_goods.is_show='1' AND v_ts.is_show='1'";
          //  $where.=" ";
            $select="v_favorite.user_id,v_favorite.ts_id,v_ts.banner_product as banner_image,v_ts.title,v_ts.tag,v_goods.goods_id,v_goods.ori_price AS shop_price,v_ts.order_sell AS goods_buy";
            $data['list_join']=$this->User_model->get_favoreite_trip($select,$where,$table='v_favorite');
            foreach($data['list_join'] as $k=>$v)
            {
                $data['list_join'][$k]['product_url']=base_url("myshop/trip_detail?ts_id=$v[ts_id]&menu=1");
                $data['list'][]=$data['list_join'][$k];
            }
            unset($data['list_join']);
		}
		else
		{
			...
			foreach($data['list'] as $k=>$v)
			{
				...
				$data['list'][$k]['shop_url']=base_url("bussell/business_info_app?business_id={$v['business_id']}&menu=1");
			}
	...
	public function user_info_new()
	{
		...
		else
		{
			...
			else
			{
                ...
                if($data['is_carmer'] == '1')
                {
                    $res = $this->User_Api_model->comment_select(' id_range '," user_id=$user_id  ",'','',0,1,'v_auth_camer');
                    if($res)
                    {
			...
			$data['video_list'] = $list;
            $data['json_share']=array(
                'url'=>base_url("Admin/user_info_share?user_id=$user_id"),
                'image'=>$data['image'],
                'title'=>$data['user_name'],
                'desc'=>'一站式旅游服务平台，为您精挑细选，让你坐享其成',
            );
            $rs=$this->User_model->get_select_one('business_id,discount,logo_image_thumb,business_name,business_country,star_num,tag',array('user_id'=>$user_id,'is_show'=>'1'),'v_wx_business');
            if($rs!='0'){
                $data['shop']['business_id']=$rs['business_id'];
                $data['shop']['discount']=$rs['discount'];
                $data['shop']['en_dis']=(string)((1000-$rs['discount']*100)/10).'%';

                if(stristr($rs['logo_image_thumb'], 'http')===false)
                {
                    $rs['logo_image_thumb']= $this->config->item('base_url') . ltrim($rs['logo_image_thumb'],'.');
                }
                $data['shop']['image']=$rs['logo_image_thumb'];
...
//            $left_table='v_goods',$left_title="v_activity_children.act_id=v_goods.act_id",$sum=false,$L=1, $start=0,$page_num=3);
        ...
        {
            if (stristr($v['act_banner'], 'http') === false)
            {
                $data['temp_products'][$k]['act_banner'] = $this->config->item('base_url') . ltrim($v['act_banner'], '.');
            }
            if (stristr($v['ts_banner'], 'http') === false)
            {
                $data['temp_products'][$k]['ts_banner'] = $this->config->item('base_url') . ltrim($v['ts_banner'], '.');
            }

            if($v['act_id']>0)
            {
                $range_id=$this->User_model->get_select_all('range_id',array('act_id'=>$v['act_id'],'is_show'=>'1','type'=>'0'),'ar_id','ASC','v_act_range');
                $range_name=array();
                if(is_array($range_id))
                {
                    foreach($range_id as $k1=>$v1)
                    {
                        $range_name[]=$this->User_model->get_location_name($v1['range_id']);
                    }
                ...
                $data['products'][]=array(
                    'act_id'=>$v['act_id'],
                    'ts_id'=>'0',
                    'star_num'=>$v['act_star_num'],
                    'title'=>$v['act_title'],
                    'goods_buy'=>$v['act_order_sell'],
                    'image'=> $data['temp_products'][$k]['act_banner'],
                    'ori_price'=>$v['ori_price'],
                    'oori_price'=>$v['oori_price'],
                    'front_price'=>'0',
                    'range_name'=>$range_name,
                    'url'=>base_url("myshop/products_detail_new?act_id=$v[act_id]&menu=1")
                );
            }
            else
            {
                $data['products'][]=array(
                    'ts_id'=>$v['ts_id'],
                    'act_id'=>'0',
                    'star_num'=>$v['ts_star_num'],
                    'title'=>$v['ts_title'],
                    'goods_buy'=>$v['ts_order_sell'],
                    'image'=> $data['temp_products'][$k]['ts_banner'],
                    'ori_price'=>$v['ori_price'],
                    'oori_price'=>$v['oori_price'],
                    'front_price'=>$v['front_price'],
                    'range_name'=>$this->User_model->get_location_name($v['ts_range_id']),
                    'url'=>base_url("myshop/trip_detail?ts_id=$v[ts_id]&menu=1")
                );
            }
        }
     ...
     //   $where  = " u.auth='1' ";
        ...
        $where="  is_hot='1'";
        //base64_encode()
        $select="name,name_en,name_pinyin,lat,lng,lat AS latitude,lng AS longitude,image,id AS range_id ";

        $data['location']=$this->User_model->get_select_more($select,$where,$start=0,$page_num=4,'displayorder',$order='ASC',$table='v_location');
        foreach($data['location'] as $k=>$v)
        {
            $data['location'][$k]['name_base']=base64_encode($v['name_en']);
        }
        $this->data_back($data, '0x000');  //返回数据
    }
    //目的地产品搜索
    public function products_search1()
    {
        ...
        $range = isset($_REQUEST['range']) ? (int)trim($_REQUEST['range']) : '0';

        $page=trim($this->input->get_post('page',TRUE));
        if(!$page){
            $page=1;
        }
        $select="v_activity_children.star_num,v_activity_children.act_id,v_wx_business.business_id,v_activity_children.special,
        ...
        $where="v_activity_children.special IN ('4','5','6','7')
        AND v_activity_children.is_show ='1'
        AND v_goods.is_show = '1'
        ";
        if($title)
        {
            $where .="  AND (v_activity_children.title LIKE '%$title%' OR v_activity_children.range_name LIKE '%$title%') ";
        }
        if($range)
        {
         $where.=" AND   AND v_act_range.range_id =$range";
        }
        $page_num =5;
        $start = ($page-1)*$page_num;

        $data['products']=$this->User_model->get_special($select,$where,$start,$page_num);
        foreach($data['products'] as $k=>$v)
        {
    ...
    public function products_search()
    {
        ...
        $title=trim($this->input->get_post('title',TRUE));
        $range=(int)($this->input->get_post('range',TRUE));
        if($title)
        {
            $where=" AND  (v_goods.goods_name LIKE '%$title%')";
        }
        else
        {
            if($range)
            {
                $range_arr=$this->User_model->get_select_all('act_id,ts_id',array('range_id'=>$range,'is_show'=>'1'),'ar_id','ASC','v_act_range');
                $act_arr=array();
                $ts_arr=array();
                ...
                $act_arr=implode(',',array_unique($act_arr));
                $ts_arr=implode(',',array_unique($ts_arr));
                $where=" AND (v_activity_children.act_id IN ($act_arr) OR v_ts.ts_id IN ($ts_arr))";
            }
            else
            {
                $where=" AND  1=1";
        ...
        $data['temp_products']=$this->User_model->get_tsp_products($page_num,$start,$where);
        $data['products']=array();
...
//目的地首页
    ...
    {
        $range_id=$range=trim($this->input->get_post('range'));
      //  $name='普吉';
        //$range_id=trim($this->input->get_post('range_id'));
       // $range_id=2336;
     ///   $range_id=2030;
        if(!$range_id){
            $range_id=0;
      ...
      //  $data['banner']=$this->User_model->get_select_more($select='title,banner_hot as image,act_id',$where="is_show='1'   AND range_name LIKE'%$name%'",$start=0,$page_num=5,'act_id',$order='ASC',$table='v_activity_children');
        //$all_act_id=$this->User_model->get_select_all('act_id',array('is_show'=>'1'),'act_id','ASC','v_v_act_range');
        $data['banner']=$this->User_model->get_select_all('v_activity_children.title,v_activity_children.banner_hot as image,v_activity_children.act_id',"v_activity_children.is_show='1' AND v_act_range.range_id=$range_id AND v_act_range.is_show='1'",'v_activity_children.displayorder','ASC','v_activity_children',1,$left_table='v_act_range',$left_title="v_act_range.act_id=v_activity_children.act_id",$sum=false,$L=1, $start=0,$page_num=5);
      //  echo '<pre>';print_r($data['banner']);exit();
        if(!is_array($data['banner']))
        {
            $data['banner']=array();
            $data['local_products']='0';
            $data['special_line']='0';
            $data['traffic_products']='0';

        }else{
            $where_local="v_activity_children.is_show='1' AND  v_act_range.range_id=$range_id  AND v_act_range.is_show='1' AND v_activity_children.special='5'";
            $data['local_products']=(string)($this->User_model->get_range_count($where_local));

            $where_local="v_activity_children.is_show='1' AND  v_act_range.range_id=$range_id  AND v_act_range.is_show='1' AND v_activity_children.special='4'";
            $data['special_line']=(string)($this->User_model->get_range_count($where_local));

            $where_local="v_activity_children.is_show='1' AND  v_act_range.range_id=$range_id   AND v_act_range.is_show='1' AND v_activity_children.special='6'";
            $data['traffic_products']=(string)($this->User_model->get_range_count($where_local));
//           if($this->input->get('test'))
//           {
//               echo '<pre>';
//               echo $this->db->last_query();
...
//           }
        ...
        $data['ts_products']=$this->User_model->get_count("range_id=$range_id AND is_show='1'", $table='v_ts');
        $data['ts_products']=(string) $data['ts_products']['count'];

        $data['video_count']=$this->User_model->get_count("location_id=$range_id AND (is_off='1' OR is_off='0')", $table='v_video');
        $data['video_count']= (string) $data['video_count']['count'];
//        $temp=array();
//        foreach($data['banner'] as $k=>$v)
//        {
//            $data['banner'][$k]['url']=base_url("myshop/products_detail_new?act_id=$v[act_id]&menu=1");
//            $temp[]="'".$v['act_id']."'";
//        }

        $where_hot=" range_id=$range_id AND type='0' AND status='0' AND app_hot='1'";
        $data['hot']=$this->User_model->get_select_all('image_url,link_url',$where_hot,'v_banner.displayorder','ASC','v_banner');

        if($data['hot']!=0)
        {
            foreach($data['hot'] as $k=>$v)
            {
                $data['hot'][$k]['url']=$v['link_url'];
                $data['hot'][$k]['name']='热推';
                $data['hot'][$k]['desc']='大家都在关注什么！';
                $data['hot'][$k]['image']=$v['image_url'];
            }
        }else{
            $data['hot']=array();
        }

        $where_hot=" range_id=$range_id AND type='0' AND status='0' AND app_feat='1'";
        $data['rec']=$this->User_model->get_select_all('image_url,link_url',$where_hot,'v_banner.displayorder','ASC','v_banner');
        ...
        if($data['rec']!=0)
        {
            ...
            {
                $data['rec'][$k]['url']=$v['link_url'];
                $data['rec'][$k]['name']='特色';
                $data['rec'][$k]['desc']='当地有什么良品！';
                $data['rec'][$k]['image']=$v['image_url'];
            }
    ...
    public function video_list()
    {
        ...
        $range_en = isset($_REQUEST['range_en']) ? trim($_REQUEST['range_en']) : '';
        $name_base = isset($_REQUEST['name_base']) ? base64_decode(trim($_REQUEST['name_base'])) : '';
        $sign=$this->input->get('sign');
        if(!$range OR (!$range_en AND !$name_base)){
          //  $this->data_back(array(), '0x002','fail');
        }
        if($title)
        {
        ...
        else
        {
            if(stristr($_SERVER['HTTP_USER_AGENT'],'android'))
            {
                ...
                $where  = " 1=1 AND $time > start_time AND types=1 AND (title LIKE '%$range%' OR all_address LIKE '%$range%' OR address  LIKE '%$range%'  OR address  LIKE '%$name_base%'  OR address  LIKE '%$name_base%' OR title  LIKE '%$name_base%')";

            }
            $where  = " 1=1 AND $time > start_time AND types=1 ";
            if($range)
            {
                $where.="  AND location_id=$range";
            }
        ...
        $video = $this->User_Api_model->video_list($where,
            ...
            $this->config->item('catch_time'));
        if($sign=='9BE11279A280A879FDA9F52D63448102')
        {
        ...
        if(!empty($video))
        {
            ...
            foreach ($video as $key => $value) {
                //视频截图尚未生成时，列表暂不显示
                //if(!file_exists('/opt/nginx/html/zxqc/api'.ltrim($value['image'],'.')) || (!@fopen('http://oss.etjourney.com/etjourney/'.$value['video_name'].'.m3u8','r') && $value['is_off']=='1'))
                //if(!file_exists('/opt/nginx/html/zxqc/api'.ltrim($value['image'],'.')))
               // if(!file_exists('/opt/nginx/html/zxqc/api'.ltrim($value['image'],'.')) || ($value['stop_time'] != '0' && $value['push_type'] == '0' && (time()-intval($value['stop_time']) < 200) && $value['is_off']=='1'))
                if(!file_exists('/opt/nginx/html/zxqc/api'.ltrim($value['image'],'.')) )
                {
                    continue;
                }
                ...
                else
                {
                    ...
                    //cdn测试
                    if($value['user_id']=='1085' || $value['user_id']=='1744' || $value['user_id']=='4011' || $value['user_id']=='1743')
                    {
                    	$info['rtmp'] = 'rtmp://pull-rtmp.etjourney.com/live/'.$value['video_name'];
                    }
                }
                $user_info = $this->User_Api_model->user_info(" user_id=$value[user_id] ",'v_users',$this->config->item('catch'),$value['user_id'].'_user_info',$this->config->item('catch_time'));
                //$data[$key]['avatar'] = $user_info['image'];
...
//tctype 1护肤2特产3彩妆4药品
    ...
    public function get_filter_photo_words()
    {
        ...
        //请选择<婚纱摄影 亲子旅拍 海外旅拍 闺蜜摄影 写真摄影 全家福
        $data['type']=array(
            array('name'=>'全部分类','index'=>'0'),
            array('name'=>'婚纱摄影','index'=>'1'),
            array('name'=>'亲子旅拍','index'=>'2'),
            array('name'=>'海外旅拍','index'=>'3'),
            array('name'=>'闺蜜摄影','index'=>'4'),
            array('name'=>'写真摄影','index'=>'5'),
            array('name'=>'全家福','index'=>'6'),
        );
        $data['tctype']=array(
            array('name'=>'全部分类','index'=>'0'),
            array('name'=>'婚纱摄影','index'=>'1'),
    ...
    public function get_trip_detail()
    {
        ...
        if(stristr($_SERVER['HTTP_USER_AGENT'],'iphone'))
        {
            $type=trim($this->input->get_post('tctype'));
        }else{
        ...
        $range_name=$range=trim($this->input->get('range'));
        $range=$this->User_model->putchange($range);
        if(!$type){
            $type=0;
        }
      ...
      //  $range='普吉';
        ...
        $whereinfo="v_ts.is_show='1'   AND range_id =$range AND v_goods.is_show='1' AND v_ts.ts_id>'1'";
        if($type){
            $whereinfo.=" AND v_ts.type='$type'";
        }
        if($title){
            $whereinfo="  v_goods.is_show='1' AND v_ts.is_show='1'   AND (v_ts.range_name LIKE'%$title%' OR v_ts.title LIKE'%$title%' )";
        }
        $select="v_goods.ori_price,v_goods.oori_price,v_ts.range_name,v_ts.title,v_ts.banner_image as image,v_ts.ts_id,v_ts.order_sell,v_goods.front_price,v_ts.star_num,
        ...
        if( $data['info']==false){
            ...
            foreach( $data['info'] as $k=>$v){
                if(mb_strlen($v['title'])>18){
                    $data['info'][$k]['title']=mb_substr($v['title'],0,18);
                }
                $data['info'][$k]['url']=base_url("myshop/trip_detail?ts_id=$v[ts_id]&menu=1");
                $data['info'][$k]['range_name']=$range_name;
            }
...
//lp 筛选
    public function get_trip_ts_detail()
    {
        ...
        foreach($data['attr_json'] as $k=>$v)
        {
           ...
           // $data['date_val'][$k]=$v;
            foreach($v['radio'] as $k2=>$v2)
            {
                ...
                $data['date_val'][$k]['str_price']=$v['str_price'];
     ...
     {
         $user_id_buy=$this->input->get('user_id',TRUE);


         $ts_id=$this->input->get_post('ts_id',TRUE);
       //  $user_id_sell=$this->input->get('user_id_sell',TRUE);


         $amount=1;
         $check_all_num=$this->input->get_post('number',TRUE);

         $date=$this->input->get_post('date',TRUE);
         $photo_time=$this->input->get_post('photo_time',TRUE);

         $photo_user_id=$this->input->get_post('photo_user_id',TRUE);
         $photo_user_price=$this->input->get_post('photo_user_price',TRUE);
         $str_price=$this->input->get_post('str_price',TRUE);

         $is_front=$this->input->get_post('is_front',TRUE);
         if($is_front==1){
            $from='6';
         }else{
            $from='7';
         }
         if(!$user_id_buy OR !$ts_id  OR !$check_all_num OR !$date OR !$photo_time OR !$photo_user_id)
         {
             $this->data_back("参数不对", '0x002','fail');
         }

         $order_sn=$this->get_order_sn();
         $user_id_buy_name=$this->User_model->get_username($user_id_buy,'v_users');
         $name=$this->User_model->get_selecr_one('photo_starttime,photo_endtime',array('user_id'=>$photo_user_id,'v_users'));
         $row=$this->User_model->get_one('v_ts.user_id,v_goods.goods_id,v_ts.title as goods_name,v_goods.shop_price',array('v_ts.ts_id'=>$ts_id,'v_goods.is_show'=>'1'),'v_ts','v_goods','ts_id','ts_id');
         $user_id_sell=$row['user_id'];
         $user_id_sell_name=$this->User_model->get_username($user_id_sell,'v_users');
         if( (strtotime($date) >=$name['photo_starttime'] AND strtotime($date)<=$name['photo_endtime'])  OR ( strtotime($date)+$photo_time>=$name['photo_starttime'] AND strtotime($date)+$photo_time<=$name['photo_endtime']))
         {
             $this->data_back("该套系，摄影师已被选择", '0x002','fail');
         }
         else
         {
             ...
             $checl_arr=array('date'=>$date,'user_id'=>$photo_user_id,'user_price'=>$photo_user_price,'str_price'=>$str_price);
             $checl_arr=json_encode($checl_arr);
             ...
             $goods_order_arr=array(
                 ...
                 'goods_attr_id'=>$checl_arr

             );
             $this->User_model->user_insert('v_order_goods', $goods_order_arr);

             $this->User_model->update_one(array('user_id'=>$photo_user_id),array('photo_starttime'=>strtotime($date),'photo_endtime'=>strtotime($date)+$photo_time),'v_users');
             $data['order_id'] = $order_id;
    ...
    //获取摄影师词语
    public function get_photo_words()
    {
        $data['order']=array(
            array('name'=>'默认排序','index'=>'1'),
            array('name'=>'销量最高','index'=>'2'),
        );
    ...
    //获取 摄影师
    public function get_photoer()
    {
        $user_id=trim($this->input->get_post('user_id'));

        $fans_user_id=array();
        $fans_count=$this->User_model->get_select_all($select='fans_id',array('user_id'=>$user_id),$order_title='f_id',$order='ASC',$table='v_follow');
        if($fans_count!==false){
           foreach($fans_count as $k=>$v)
           {
               $fans_user_id[]=$v['fans_id'];
           }
        }
        ...
        $whereinfo="  v_auth_camer.id_camer_status='2' ";
        if($range)
        {
            $whereinfo.=" AND  v_auth_camer.id_range ='$range' ";
        }

        if($type){
            $whereinfo.=" AND v_auth_camer.tag LIKE '%$type_words%'";
        }
        $select="v_users.user_id,v_users.user_name,v_users.image as avatar,v_users.sex,v_users.pre_sign,v_auth_camer.tag";

        $data['info']=$this->User_model->get_photoer($select,$whereinfo,$start,$page_num,$order);
        //$data['info']=array_unique( $data['info']);
        foreach($data['info'] as $k=>$v)
        {
            if (stristr($v['avatar'], 'http') === false)
            {
                $data['info'][$k]['avatar'] = $this->config->item('base_url') . ltrim($v['avatar'], '.');
            }
 //           $temp=$this->User_model->get_select_all('type_id,type_name',"user_id=$v[user_id]","id",'ASC','v_ts_istype');
//            foreach($temp as $k1=>$v1)
//            {
//                $data['info'][$k]['type_arr'][]=array('type_id'=>$v1['type_id'],'name'=>$v1['type_name']);
//            }
            $data['info'][$k]['type_arr_temp']=explode(',',$v['tag']);

            foreach($data['info'][$k]['type_arr_temp'] as $k1=>$v1)
            {
                $data['info'][$k]['type_arr'][]=array('type_id'=>'1','name'=>$v1);
            }
            if(in_array($v['user_id'],$fans_user_id))
            {
                $data['info'][$k]['is_follow']='1';
            }else{
                $data['info'][$k]['is_follow']='0';
            }

            $data['info_new'][$v['user_id']]= $data['info'][$k];
        }
        ...
        if( $data['info']==false){
            ...
            $data['info_new']=array_values($data['info_new']);
            $data['info']=$data['info_new'];
            unset($data['info_new']);
        }
        $this->data_back($data, '0x000');  //返回数据
    }
