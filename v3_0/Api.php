	...
	public function common_login()
	{
       ...
       // echo '<pre>';print_r($userinfo);
		...
		else
		{
            ...
            if($userdevice && !in_array($userinfo[0]['user_id'],array(1077,2025,4011,1,5783)))
            {
...
//
	...
	public function user_info_new()
	{
		...
		else
		{
            ...
            $rs=$this->User_model->get_select_one('business_id,discount,logo_image_thumb,business_name,business_country,star_num,tag,is_show',array('user_id'=>$user_id,'is_show'=>'1'),'v_wx_business');
            if($rs!='0'){
...
//            $left_table='v_goods',$left_title="v_activity_children.act_id=v_goods.act_id",$sum=false,$L=1, $start=0,$page_num=3);
        ...
        $where_add=" AND type='6'";
        $data['shop']=$this->Shop_model->get_shop_list(4,0,$where_add);
        //echo $this->db->last_query();
        $where="  is_hot='1'";
...
//        //请选择<婚纱摄影 亲子旅拍 海外旅拍 闺蜜摄影 写真摄影 全家福
        $data['type']=array(
            array('name'=>'全部分类','index'=>'63'),
            array('name'=>'婚纱摄影','index'=>'1'),
            array('name'=>'亲子旅拍','index'=>'2'),
            array('name'=>'海外旅拍','index'=>'4'),
            array('name'=>'闺蜜摄影','index'=>'8'),
            array('name'=>'写真摄影','index'=>'16'),
            array('name'=>'全家福','index'=>'32'),
        );
        $data['tctype']=array(
            array('name'=>'全部分类','index'=>'0'),
            array('name'=>'婚纱摄影','index'=>'1'),
    ...
    public function get_trip_detail()
    {
        ...
        $type=trim($this->input->get_post('tctype',true));
        ...
        if(stristr($_SERVER['HTTP_USER_AGENT'],'iphone'))
        {
            $range_name=trim($this->input->get('range'));
            if(!$range_name)
            {
                $range_id=0;
            }else{
                $range_id=$this->User_model->putchange($range_name);
            }

        }else{
            $range_id=trim($this->input->get('range',true));
        }
      ...
      //  $range='普吉';
        $whereinfo="v_ts.is_show='1'    AND v_goods.is_show='1' ";
        if($range_id)
        {
            $whereinfo.=" AND  range_id =$range_id";
        }
...
//
    ...
    public function get_shop()
    {
        $user_id=trim($this->input->get_post('user_id',true));
            $type=trim($this->input->get_post('tctype',true));
        $page=trim($this->input->get_post('page',true));
        $range=trim($this->input->get_post('range',TRUE));
        $order_type=trim($this->input->get_post('order',TRUE));
        if($order_type)
        {
            $order_type=1;
        }
        ...
        if($range)
        {
           ...
           // $where_add.= " AND business_id IN (select business_id from v_ts where range_id=$range AND is_show='1')";
        }
        if($type)
        {
            $where_add.=" AND (ts_type & $type) = $type";
        }


        $data=$this->Shop_model->get_shop_list($page_num=4,$start,$where_add,$order_type);
      //  echo $this->db->last_query();
    ...
    public function check_code()
    {
        ...
        $url=$this->common->url_decode($url);
        $url_arr=json_decode($url,true);
        if(is_array($url_arr))
        {
            if($url_arr['type']==1)
            {
                ...
                $goods_id=$url_arr['id'];
                $where="v_order_goods.goods_id =$goods_id AND v_order_info.user_id_buy=$user_id AND v_order_info.order_status IN ('1','2','3') AND v_order_info.date LIKE '%$date%'";
                //$where="v_order_goods.goods_id =$goods_id AND v_order_info.user_id_buy=$user_id AND v_order_info.order_status IN ('1','2','3')";
            ...
            elseif($url_arr['type']==2)
            {
                $this->data_back("参数有误", '0x011','fail');
            }
            else
            {
                $this->data_back("参数有误", '0x011','fail');
