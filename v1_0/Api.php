...
class Api extends My_Controller {
	...
	*/
	public function avatar_upload()
	{
        ...
        $image=$this->imagecropper($image,$user_id);
    ...
    public function imagecropper($source_path='./public/images/1265/id_driver.jpg',$user_id,$key2='time',$target_width='220', $target_height='220')
    {
        ...
        if (!file_exists('./public/images/user/' . $user_id)) {
            if (!mkdir('./public/images/user/' . $user_id)) {
                return FALSE;
            }
        }
        // 源图过高
        ...
        if ($source_mime == 'image/jpeg') {
            ...
            $new_image = './public/images/user/' . $user_id . '/' . $key2 . '.' . $type;
	...
	**/
	...
	{
		$data = $this->User_Api_model->comment_select('title,image_url,link_url'," status=0 AND range_id='0' ",'','displayorder',0,10,'v_banner');
		if($data)
		{
			foreach($data as $key => $value)
			{
		...
		if($type==1)
		{
          ...
          //  $where.="  AND v_goods.is_show='1'";
			...
			foreach($data['list'] as $k=>$v)
			{
				...
				$data['list'][$k]['product_url']=base_url("myshop/products_detail_new?act_id=$v[act_id]");
			}
          ...
          //  $where.=" ";
            ...
            {
                $data['list_join'][$k]['product_url']=base_url("myshop/trip_detail?ts_id=$v[ts_id]");
...
//目的地首页
    ...
    {
        $name=trim($this->input->get_post('range'));
        $name='普吉';
...
//tctype 1护肤2特产3彩妆4药品
    ...
    public function get_trip_detail()
    {
        ...
        $select="v_goods.ori_price,v_goods.oori_price,v_ts.range_name,v_ts.title,v_ts.banner_hot as image,v_ts.ts_id,v_ts.order_sell,v_goods.front_price,v_ts.star_num,
        ...
        if( $data['info']==false){
            ...
            foreach( $data['info'] as $k=>$v){
                ...
                $data['info'][$k]['url']=base_url("myshop/trip_detail?ts_id=$v[ts_id]");
            }
...
//lp 筛选
    ...
    public function get_photoer()
    {
        ...
        if(!$user_id){
            //$this->data_back('参数不足', '0x002','fail');
        ...
        $whereinfo=" v_auth_camer.id_range LIKE'%$range%' AND v_auth_camer.id_camer_status='2' ";
        if($type){
            $whereinfo.=" AND v_ts_istype.type_id='$type'";
        }
        $select="v_users.user_id,v_users.user_name,v_users.image as avatar,v_users.sex,v_users.pre_sign";
