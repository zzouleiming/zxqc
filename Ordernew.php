   ...
   public function user_id_and_open_id(){
    ...
    public function discount_app_buy($page=1)
    {
       ...
       // $this->Order_model->judeg_time_out();
        ...
        $where="user_id_buy=$user_id  AND (from='3' OR from='2') AND is_show='1'";
        $page_num =5;
        $data['now_page'] = $page;
        $count = $this->User_model->get_count($where,'v_order_info');
        $data['max_page'] = ceil($count['count']/$page_num);
    ...
    public function discount_app_sell($page=1){
     ...
     //   $this->Order_model->judeg_time_out();
      //  $this->Order_model->sure_sub_discount();
        ...
        $data['user_name']=$row['user_name'];
        $data['image']=$row['image'];
        $where="user_id_sell=$user_id   AND (from='2' OR from='3')  AND is_show='1'";
        $page_num =5;
        $data['now_page'] = $page;
        $count = $this->User_model->get_count($where,'v_order_info');
        $data['max_page'] = ceil($count['count']/$page_num);
    ...
    public function order_cancle_buy(){
        ...
        if( $this->Order_model->order_update($where,$arr_update,$table='v_order_info')){
           /* $row=$this->User_model->get_select_all('device_id,type',array('user_id'=>$user_id_sell),'phone_id', 'ASC','v_device');
            ...
            $rs=$this->User_model->get_select_one('user_id_buy,pid',array('order_id'=>$order_id),'v_order_info');
            if($rs['pid']>0)
            {
                $this->Order_model->order_update(array('order_id'=>$rs['pid']),$arr_update,$table='v_order_info');
            }
    ...
    public function order_cancle_sell(){
        ...
        if( $this->Order_model->order_update($where,$arr_update,$table='v_order_info')){
           /* $row=$this->User_model->get_select_all('device_id,type',array('user_id'=>$user_id_buy),'phone_id', 'ASC','v_device');
            ...
            $rs=$this->User_model->get_select_one('user_id_sell,pid',array('order_id'=>$order_id),'v_order_info');
            if($rs['pid']>0)
            {
                $this->Order_model->order_update(array('order_id'=>$rs['pid']),$arr_update,$table='v_order_info');
            }
 ...
 */
    ...
    public function order_eval_sub(){
        ...
        $this->Order_model->com_order($user_id,$data);
...
//添加注意事项?
    public function put_attention()
    {
        ...
        $order_info=$this->Order_model->get_order_detail($order_id);
        ...
        if($order_info['pid']!=0)
        {
            $this->User_model->update_one(array('order_id'=>$order_info['pid']),array('attention'=>$attention,'order_status'=>'2','shipping_time'=>time()),'v_order_info');

        }
        $str="你购买的产品{$order_info['goods'][0]['goods_name']}已发货，买家留言{$attention},如有疑问，请随时与我们联系";
        $data=array(
            'user_id'=>$order_info['user_id_buy'],
            'message'=>$str,
            'add_time'=>time(),
        );
       $this->User_model->user_insert($table='v_prompt',$data);
        echo 1;
    }
   ...
   public function order_detail()
   {
       ...
       $order_id=$this->input->get('order_id',TRUE);
       $data=$this->User_model->get_select_one('cn_name,en_first_name,en_oth_name,cn_mobile,weixin,mail,en_hotel,en_hotel_address,commont',array('order_id'=>$order_id,'type'=>'1'),'v_order_addition');
      ...
      // $rs3=$this->User_model
       ...
       $data['goods_url']=base_url("myshop/products_detail_new?act_id=$rs2[act_id]");
     ...
     //  $data['attr']=$rs['goods_attr_id'];
       ...
       $data['count']=$count['count'];
        $data['people_detail']=base_url('ordernew/oreder_detail_bc');
        $data['people_detail']="javascript:void(0)";
   ...
   public function oreder_detail_bc()
   {
    ...
    public function order_list_unpaybuy($page=1){
        ...
        $whereunpaybuy="user_id_buy=$user_id And (order_status='0' OR  order_status='4') AND from IN ('0','1','4','5','6','7','8','9','10','11')  AND is_show='1' AND goods_amount!=0";
        $page_num =5;
        if($this->input->get('test')){
            $page_num =1;
        }
        ...
        $start = ($page-1)*$page_num;
        $data['unpaybuy']=$this->Order_model->get_order_list($whereunpaybuy,$start,$page_num,$user_id);
      // if($this->input->get('test')){echo "<pre>";print_r($data['unpaybuy']);exit();}
    ...
    public function order_list_unpaysell($page=1)
    {
        ...
        $data['user_name']=$row['user_name'];
        $data['image']=$row['image'];
        $whereunpaysell="user_id_sell=$user_id And (order_status='0' OR  order_status='4') AND from IN ('0','1','4','5','6','7','8','9','10','11')  AND is_show='1' AND goods_amount!=0";
        $page_num =5;
        $data['now_page'] = $page;
        ...
        $data['unpaysell']=$this->Order_model->get_order_list($whereunpaysell,$start,$page_num,$user_id);
        $type=$this->get_lan_user();
        if(in_array($type,$this->long_css)){
            $data['type']='en';
        }
    ...
    public function order_list_unshipbuy($page=1){
        ...
        $whereunshipbuy="user_id_buy=$user_id And order_status='1'  AND from IN ('0','1','4','5','6','7','8','9','10','11')  AND is_show='1' AND goods_amount!=0";
        $page_num =5;
        $data['now_page'] = $page;
       ...
       // $data['unshipbuy']=$this->Order_model->order_list_home($whereunshipbuy,$start,$page_num);
        $data['unshipbuy']=$this->Order_model->get_order_list($whereunshipbuy,$start,$page_num,$user_id);
       // echo "<pre>";print_r($data);exit();
        $type=$this->get_lan_user();
        if(in_array($type,$this->long_css)){
            $data['type']='en';
...
//旅拍补全尾款
    ...
    public function order_list_unshipsell($page=1){
      ...
      //  $user_id=1077;
        ...
        $data['user_name']=$row['user_name'];
        $data['image']=$row['image'];
        $whereunshipsell="user_id_sell=$user_id And order_status='1' AND  (from='0' OR from='1' OR from='4' OR from='5' OR from='10' OR from='11')  AND is_show='1' AND goods_amount!=0";
        $page_num =5;
        $data['now_page'] = $page;
        ...
        $start = ($page-1)*$page_num;
        $data['unshipsell']=$this->Order_model->get_order_list($whereunshipsell,$start,$page_num,$user_id);
        $type=$this->get_lan_user();
        if(in_array($type,$this->long_css)){
            $data['type']='en';
        }
    ...
    public function order_list_unconfimbuy($page=1){
        ...
        $data['user_name']=$row['user_name'];
        $data['image']=$row['image'];
        $whereunconfimbuy="user_id_buy=$user_id And (order_status='2' OR  order_status='3') AND  (from='0' OR from='1' OR from='4' OR from='5' OR from='10' OR from='11')  AND is_show='1' AND goods_amount!=0";
        $page_num =5;
        $data['now_page'] = $page;
    ...
    public function order_list_unconfimsell($page=1){
        ...
        $data['user_name']=$row['user_name'];
        $data['image']=$row['image'];
        $whereunconfimsell="user_id_sell=$user_id And (order_status='2' OR  order_status='3') AND  (from='0' OR from='1' OR from='4' OR from='5' OR from='10' OR from='11')  AND is_show='1' AND goods_amount!=0";
        $page_num =5;
        $data['now_page'] = $page;
        $count = $this->User_model->get_count($whereunconfimsell,'v_order_info');
        $data['unconfimsellmax_page'] = ceil($count['count']/$page_num);
        ...
        $data['unconfimsell']=$this->Order_model->get_order_list($whereunconfimsell,$start,$page_num);
        $type=$this->get_lan_user();
        if(in_array($type,$this->long_css)){
            $data['type']='en';
        }
    ...
    public function order_detail_ts($order_id)
    {
        ...
        if($data['order_info']['from']=='6'){
            $data['order_info']['pay_amount'] = $data['order_info']['front_amount'];
        }elseif($data['order_info']['from']=='7' && $data['order_info']['pid']!='0'){
            $data['order_info']['pay_amount'] = floatval($data['order_info']['goods_amount']) - floatval($data['order_info']['front_amount']);
        }else{
            $data['order_info']['pay_amount'] = $data['order_info']['goods_amount'];
        }
        //echo "<pre>";print_r($data);exit();
