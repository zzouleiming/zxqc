...
class Addviews extends My_Controller {
	...
	public function index()
	{
		...
		//$cip ='42.121.5.3';
		/*获取当前直播列表*/
		$video_list = $this->User_Api_model->comment_select('video_id,video_name,start_time,socket_info'," is_off=0 AND start_time<$time_1 AND types='1'",'','video_id DESC','','','v_video',1);
		...
		if($video_list)
		{
			foreach($video_list as $video)
			{
				...
				if($time_2 > 20 && $time_2 < 60)
				{
					...
					/*直播第2,3分钟每分钟加入6人*/
					if(in_array($batch,array(2,6,10,14,18,20)))
					{
						$add_view = '1';
					}
				}elseif($time_2 > 180){
				...
				if ($add_view) {
                    ...
                    /* 获取当前可用robot用户 */
                    $res = $this->User_Api_model->comment_select(' user_id,user_name,image,auth,rand() as rd ', " groupid=4 ", '', ' rd ', 0, 1, 'v_users');
                    if ($res) {
                        //echo '<pre>';
                        $user_id = intval($res[0]['user_id']);
                        //var_dump($res);
                    ...
                    /* 更新当前直播观看数信息 */
                    $this->User_Api_model->update_views(array('video_id' => $video['video_id']), 'v_video', 1);
                }
                /*robot发表随机评论*/
				if($add_comm)
				{
