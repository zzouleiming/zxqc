...
class Package_tour extends MY_Controller
{
    ...
    public function view($page_id){
        ...
        $where= array(
         ...
         $data['u']=$this->input->get('u',true);
		 $data['m']=$this->input->get('m',true);
  ...
  //      $is_pay = $page_info['is_pay'] ? $page_info['is_pay'] : '';
        ...
        $this->load->view('home/package_tour/'.$page_info['template_type'].'/index', $data);
        $this->show_count();
    }
    ...
    public function view_data($page_id){
		...
		if (!empty($m)) {
			$m = explode('@',$m);
            $data['service']['user']['mobile'] = $m[0];
            $data['service']['user']['wx'] = $m[1];
            $data['service']['user']['user_name'] = $m[2]?$m[2]:'';
        }
        $data['data'] = array();
        $data['data']['page_info'] = array(
			...
			'h5_home' => json_decode($page_info['h5_home']),
            ...
            'style'=>$page_info['template_type'].'/'.$page_info['style_typle'],
             'business_id'=> $page_info['business_id'],
    ...
    private function _page_info($page_id){
        ...
        $page_info['max_price'] = max(array_values($page_info['date_choose']));
        if($page_info['max_price']> $page_info['min_price']){
           $page_info['min_price']=$page_info['min_price'].'起'; 
        }
        ...
        foreach($city_list as $key => $val){
            ...
            foreach($packages as $key1 => $val1){
                ...
                foreach($trips as $key2 => $val2){
                    ...
                    $package_list[$key]['package'][$key1]['trip'][$key2]['food'] = json_decode($val2['food'], true);
    ...
    private function _cal_price($date_price){
        ...
        foreach($later_date_choose as $key => $val){
            ...
            $later_min_price[$key] =(max($val)>min($val)?min($val).'起':min($val));
        ...
        foreach($cal_info as $key => $val){
			...
			if($cal[$key]['min_price'] == 0){
				unset($cal[$key]);
			}
        }
        $cal_price['cal'] = array_merge($cal) ;
        return $cal_price;
    }
    ...
    private function _get_trip_list_by_package($package_id){
        ...
        foreach($trip_list as $key => $val){
		   ...
		   $trip_list[$key]['day_content'] = $val['day_content'];
        }
        return $trip_list;
    }
