...
define('TRAVEL_FILE_KEY', 'Travel_schedule');
    ...
    public function index() {
       ...
       $data['menu_list']= $this->_get_menu();
        ...
        $business_id = $business_account['business_id'];
          $data['is_us']=$business_account['is_us'];
          
        $group_id=$business_account['group_id'];
        if($data['is_us']!=1){
            if(!$group_id){
               $where = array('is_show' => 0, 'page_type' => 1, 'limit' => 20, 'offset' => 0,'is_release'=>0, 'order_by' => 'page_id desc');   
            }else{
                $where = array('is_show' => 0, 'group_id'=>$group_id, 'page_type' => 1, 'limit' => 20, 'offset' => 0,'is_release'=>0, 'order_by' => 'page_id desc');  
            }
...
//        }
        ...
        if (!empty($data['param']['page_id'])) {
          ...
          if (!empty($data['param']['class_name'])) {
            $where['class_name'] = $data['param']['class_name'];
        }
        if (!empty($data['param']['per_total'])) {
            $where['limit'] = $data['param']['per_total'];
        }
         $data['class_name']=$this->class_name($business_id);
...
//echo $this->db->last_query();die;
        ...
        foreach ($data['page_info_list'] as $k => $v) {
            ...
            $data['page_info_list'][$k]['image_data']['top'] = $data['page_info_list'][$k]['image_data']['top'] ? $data['page_info_list'][$k]['image_data']['top'] : base_url('public/images/usadmin/no_head.jpg');
             ...
             $data['page_prev_url'] = base_url('home/package_tour/view/'.$page_id).'/'.$data['info']['share_url'];
    ...
    public function page_monitor_index() {
        ...
        if($data['is_us']!=1){
            if(!$group_id){
               $where = array('is_show' => 0, 'page_type' => 2, 'limit' => 20, 'offset' => 0,'is_release'=>0, 'order_by' => 'page_id desc');   
            }else{
                $where = array('is_show' => 0, 'group_id'=>$group_id, 'page_type' => 2, 'limit' => 20, 'offset' => 0,'is_release'=>0, 'order_by' => 'page_id desc');  
            }
...
// 编辑头图
    ...
    public function page_add() {
        ...
        $data['page_trip_url'] = base_url('usadmin/package_tour/page_trip');
        $data['type']=$this->temp_type();
        $data['class_name']=$this->class_name($data['business_id']);
        // 查询部门
        $where=array('business_id'=>$data['business_id'],'is_del'=>0);
        $data['group']=$this->Page_group_model->get_group_list($where);
        $this->load->view('usadmin/package_tour/page_add', $this->_set_common($data));
    }
// 分类 
    public  function class_type(){
        ...
        $res=$this->Class_type_model->save_classs_info($data);
        if($res){
             $result['code'] = 1;
            $result['msg'] = "添加成功";
    ...
    public function do_page_add() {
        ...
        $data['is_evaluate'] = $this->input->post('is_v', TRUE);
          $data['op_mobile'] = $this->input->post('op_mobile', TRUE);
            $data['group_id'] = $this->input->post('group', TRUE);
         $data['is_pay'] = $this->input->post('is_pay',true);
        $data['th_price']=$this->input->post('th_price',TRUE);
        $data['class_name']=$this->input->post('class_name',TRUE);
    ...
    public function article($page_id) {
        ...
        foreach ($data['list'] as $k => $v) {
            ...
            $data['list'][$k]['price'] = json_decode($v['pro_price_date'], TRUE);
             foreach($data['list'][$k]['price']['date'] as $k1=>$v1){
              $intcal[]=  intval($v1);
      
       
             }
         
           $data['list'][$k]['min'] = min($intcal);
            $data['list'][$k]['max'] = max($intcal);
        }
         $data['url']=$this->_free($page_id);
           $data['page_prev_url'] = base_url('home/package_'.($data['info']['page_type']=='1'?'tour':'travel').'/view/'.$page_id).'/'.$data['info']['share_url'];
         $data['page_qr_url']   = "http://api.etjourney.com/H5info_temp_zyx/qrcode_info?name=".$data['page_prev_url'];
...
//       print_r($data);die;
    ...
    public function page_release() {
        $data['page_id'] = $this->input->post('page_id', true);
        $data['is_release'] = $this->input->post('is_release', TRUE);
...
//ajax 处理 导航栏数据
    ...
    public function page_home($page_id){
        ...
        $data['menu_list']= $this->_get_menu(); 
         ...
         $h5_image=$this->Page_info_model->get_page_info_detail($where);
        $data['home_image']=  json_decode($h5_image['h5_home'],true);
        $data['page_id']=$page_id;
        $data['top_image']=  base_url('usadmin/package_tour/top_image');
        
      // echo "<pre>";
      //  print_r($data);die;
        $this->load->view('usadmin/package_tour/page_home', $this->_set_common($data));
    }
   //详情图片保存
   public function top_image(){
       $data['page_id']=$this->input->post('page_id',true);
       $data['h5_home']=$this->input->post('data',true);
       $res=$this->Page_info_model->save_page_info($data);
       if($res){
            $result['code'] = 1;
            $result['msg'] = "保存成功";
    ...
    public function do_page_edit() {
        ...
        $data['uploader'] = $this->input->post('uploader', TRUE);
         ...
         $data['class_name']=$this->input->post('class_name',TRUE);
          $data['op_mobile'] = $this->input->post('op_mobile', TRUE);
           $data['group_id'] = $this->input->post('group', TRUE);
          $data['unit']=$this->input->post('unit',TRUE);
            $data['h5_image']=$this->input->post('image6',TRUE);
        ...
        $where=array('page_id'=>$data['page_id'],'is_show'=>0);
        $bus=$this->Page_info_model->get_page_info_detail($where);
       ...
       if(empty($data['page_title'])) {
        ...
        $image_data = array();
        if (isset($_FILES['image_top']) && $_FILES['image_top']['error'] == 0) {
            $image_data['top'] = $this->upload_image('image_top', 'H5image/'.$data['page_id'],$data['page_id'].'image_top');
        } else {
            $image_data['top'] = $this->input->post('image1', TRUE);
        }
        if (isset($_FILES['image_spec']) && $_FILES['image_spec']['error'] == 0) {
            $image_data['spec'] = $this->upload_image('image_spec', 'H5image/'.$data['page_id'],$data['page_id'].'image_spec');
        } else {
            $image_data['spec'] = $this->input->post('image2', TRUE);
        }
        if (isset($_FILES['image_share']) && $_FILES['image_share']['error'] == 0) {
            $image_data['share'] = $this->upload_image('image_share', 'H5image/'.$data['page_id'],$data['page_id'].'image_share');
        } else {
        ...
        if (isset($_FILES['image_wx']) && $_FILES['image_wx']['error'] == 0) {
            $image_wx = $this->upload_image('image_wx','H5image/'.$data['page_id'],$data['page_id'].'image_wx');
        } else {
    ...
    public function page_copy() {
        ...
        unset($new_page_info['uploader']);
        unset($new_page_info['image_data']);
        unset($new_page_info['synthesis_image']);
        unset($new_page_info['h5_home']);
        unset($new_page_info['h5_image']);
        $new_page_info['uploader']=$business_account['business_account'];
        $this->db->trans_start();
  ...
  //行程排序
   ...
   public function pro_copy(){
    ...
    public function page_trip_edit($day_id) {
        ...
        $data['day_info']['food']=  json_decode($data['day_info']['food'],true);
    ...
    public function do_page_trip_edit() {
        ...
        $data_day['food']=  json_encode($datavs);
        ...
        if (isset($_FILES['day_image']) && $_FILES['day_image']['error'] == 0) {
            ...
            $data_day['day_image']=$this->input->post('image1',true);
        }
...
//模版类型
    private  function temp_type(){
        $where=array('is_del'=>0);
        $temp=$this->Temp_type_model->get_type_list($where);
        return $temp;
    }
    //分类
    private  function class_name($business_id=''){
        $where=array('business_id'=>$business_id,'is_del'=>0);
        $temp=$this->Class_type_model->get_class_list($where);
        return $temp;
    }
    //单图片处理 
