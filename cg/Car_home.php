...
//        $access = $this->_is_access();
        ...
        $cg['order_type'] = 1;
		$cg['pay_type'] = $this->input->post('pay_type', TRUE);
...
// 车购订单生成页面
	...
	public function car_modify() {
        ...
        if ($res) {
		  ...
		  $return['business_id'] = $data['business_id'];
		 $return['url'] = 'http://api.etjourney.com/H5info_temp_zyx/qrcode_infos?name='.base64_encode(base_url('cg/car_home/index/'.$data['page_id']).'?g='.$data['guide_id'].'&b='.$data['business_id'].'&r='.$data['group_id'].'&m='.$res);
		  $return['id'] = $res;
         $return['msg'] = "成功";
         return $this->ajax_return($return); 
...
// 导游后台
 public  function group_list(){
    ...
    $guide_id = $this->Car_session->get_session('guide_id');
	if(!$guide_id) $guide_id = $_SESSION['guide_id'] = $_SESSION['user_id'];
	if(!$guide_id){
		header('Location:'.base_url('home/user/login')); 
	die;
		}
